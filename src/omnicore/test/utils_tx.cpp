...
{
    CTxDestination address = DecodeDestination("C2uS5SDveHLU4oecepg8XJuizD3pMDs2m5");
    CScript scriptPubKey = GetScriptForDestination(address);
    int64_t amount = OmniGetDustThreshold(scriptPubKey);
...
{
    CTxDestination address = DecodeDestination("USZzpjkSq7xbD5BdNNSK3dY63Q8FqANENx");
    CScript scriptPubKey = GetScriptForDestination(address);
    int64_t amount = OmniGetDustThreshold(scriptPubKey);
