...
Uniasset can be configured by providing one or more optional command-line arguments:
```bash
$ uniassetd -setting=value -setting=value
```

All settings can alternatively also be configured via the `ufocoin.conf`.

Depending on the operating system, the default locations for the configuration file are:

- Unix systems: `$HOME/.ufo/ufocoin.conf`
- Mac OS X: `$HOME/Library/Application Support/UFO/ufocoin.conf`
- Microsoft Windows: `%APPDATA%/UFO/ufocoin.conf`

A typical `ufocoin.conf` may include:
```
server=1
rpcuser=omnicorerpc
...
To run and use Uniasset, no explicit configuration is necessary.
