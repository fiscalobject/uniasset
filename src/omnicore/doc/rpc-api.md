...
Uniasset is a fork of UFO, with the Omni Protocol feature support from OmniLayer for Bitcoin added as a new layer of functionality on top. As such interacting with the API is done in the same manner (JSON-RPC) as UFO Core, simply with additional RPCs available for utilizing Omni Protocol features.

As all existing UFO functionality is inherent to Uniasset, the RPC port by default remains as `9337` as per UFO.  If you wish to run Uniasset in tandem with UFO (eg. via a separate datadir) you may utilize the `-rpcport<port>` option to nominate an alternative port number.
...
| `referenceamount`   | string  | optional | a ufo amount that is sent to the receiver (minimal by default)                           |
...
Place, update or cancel a sell offer on the distributed token/UFO exchange.
...
| `amountdesired`     | string  | required | the amount of ufos desired                                                               |
...
Creates a new sell offer on the distributed token/UFO exchange.
...
Updates an existing sell offer on the distributed token/UFO exchange.
...
| `amountdesired`     | string  | required | the new amount of ufos desired                                                           |
...
Cancels existing sell offer on the distributed token/UFO exchange.
...
| `toaddress`         | string  | required | the address of the seller                                                                    |
| `propertyid`        | number  | required | the identifier of the token to purchase                                                      |
| `amount`            | string  | required | the UFO amount to send                                                                   |
...
$ omnicore-cli "omni_sendissuancecrowdsale" \
    "3JYd75REX3HXn1vAU83YuGfmiPXW7BpYXo" 2 1 0 "Companies" "UFO Mining" \
...
$ omnicore-cli "omni_sendissuancefixed" \
    "3Ck2kEGLJtZw9ENj2tameMCtS3HB7uRar3" 2 1 0 "Companies" "UFO Mining" \
...
$ omnicore-cli "omni_sendissuancemanaged" \
    "3HsJvhr9qzgRe3ss97b1QHs38rmaLExLcH" 2 1 0 "Companies" "UFO Mining" "Quantum Miner" "" ""
```
...
All ufos from the sender are consumed and if there are ufos missing, they are taken from the specified fee source. Change is sent to the fee source!
...
```bash
$ omnicore-cli "omni_createpayload_issuancefixed" 2 1 0 "Companies" "UFO Mining" "Quantum Miner" "" "" "1000000"
```
...
```bash
$ omnicore-cli "omni_createpayload_issuancecrowdsale" 2 1 0 "Companies" "UFO Mining" "Quantum Miner" "" "" 2 "100" 1483228800 30 2
```
...
```bash
$ omnicore-cli "omni_createpayload_issuancemanaged" 2 1 0 "Companies" "UFO Mining" "Quantum Miner" "" ""
```
